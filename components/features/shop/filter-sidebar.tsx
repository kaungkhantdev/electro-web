        ...
        {filterGroups.map((group) => (
          <div key={group.id}>
            <button
              ...
              className="flex items-center justify-between w-full py-2 text-left transition-colors duration-200 hover:bg-gray-50 rounded-lg px-2 -mx-2"
            >
              <span className="font-medium text-gray-900">{group.label}</span>
              <span className={`p-1 rounded-full transition-all duration-300 ${expandedGroups[group.id] ? 'bg-black rotate-180' : 'bg-gray-200'}`}>
                <ChevronDown className={`w-4 h-4 transition-colors duration-300 ${expandedGroups[group.id] ? 'text-white' : 'text-gray-600'}`} />
              </span>
            </button>
